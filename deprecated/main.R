# load isin_set
ev = read.csv("events.csv", sep = ",", as.is=TRUE)
...
for (i in 1:length(isin_set)) {
  ...
  adHoc_dates = strptime(subset(ev, isin==is)[,1], "%Y-%m-%d %H:%M:%OS") # adHoc_dates[1] is NA, why?!
  dates_in_range = c()
  
  # pick only adHoc_dates that lie within the range of our timeline
  for (j in 2:length(adHoc_dates)) {
  ...
  if (length(dates_in_range) > 0) {
    ...
    for (t in 1:length(dates_in_range)) {
      ...
      future_timestamps = rep(msg_ts, 9) + c(0, 1, 5, 10, 30, 60, 60*5, 60*10, 60*60, 2*60*60, 5*60*60)
      future_timestamps_labels = c("adHoc", "+1s", "+5s", "+10s", "+30s", "+1m", "+5m", "+10m", "+1h", "+2h", "+5h")
      ...
      barplot(diffs, future_timestamps, names.arg=future_timestamps_labels,
...
barplot(colSums(diff_set != 0)/nrow(diff_set), ncol(diff_set), ylab="% of stocks that show activity after an adHocMessage",
        names.arg=future_timestamps_labels, main="Trade activity after published adHoc message", ylim=c(0,1))
